...
class StoryFrame(wx.Frame):
    ...
    def __init__(self, parent, app, state=None, refreshIncludes=True):
        ...
        self.Bind(
            wx.EVT_MENU,
            lambda e: self.storyPanel.eachWidget(
                lambda i: i.setSelected(True, exclusive=False)
            ),
            id=wx.ID_SELECTALL,
        )
        ...
        self.Bind(
            wx.EVT_MENU, self.storyPanel.newWidget, id=StoryFrame.STORY_NEW_PASSAGE
        )
        ...
        self.Bind(
            ...
            lambda e: self.storyPanel.newWidget(
                text=self.storyPanel.FIRST_CSS, tags=["stylesheet"]
            ),
        ...
        self.Bind(
            wx.EVT_MENU,
            lambda e: self.storyPanel.newWidget(tags=["script"]),
            id=StoryFrame.STORY_NEW_SCRIPT,
        )

        self.newPassageMenu.Append(StoryFrame.STORY_NEW_ANNOTATION, "&Annotation")
        self.Bind(
            wx.EVT_MENU,
            lambda e: self.storyPanel.newWidget(tags=["annotation"]),
        ...
        self.Bind(
            wx.EVT_MENU,
            lambda e: self.storyPanel.eachSelectedWidget(lambda w: w.openEditor(e)),
            id=wx.ID_EDIT,
        )

        self.storyMenu.Append(
            StoryFrame.STORY_EDIT_FULLSCREEN, "Edit in &Fullscreen\tF12"
        )
        self.Bind(
            wx.EVT_MENU,
            lambda e: self.storyPanel.eachSelectedWidget(
                lambda w: w.openEditor(e, fullscreen=True)
            ),
        ...
        self.importImageMenu.Append(
            StoryFrame.STORY_IMPORT_IMAGE_URL, "From Web &URL..."
        )
        self.Bind(
            wx.EVT_MENU, self.importImageURLDialog, id=StoryFrame.STORY_IMPORT_IMAGE_URL
        )
        ...
        self.Bind(
            wx.EVT_MENU, self.createInfoPassage, id=StoryFrame.STORYSETTINGS_START
        )

        self.storySettingsMenu.Append(StoryFrame.STORYSETTINGS_TITLE, "StoryTitle")
        self.Bind(
            wx.EVT_MENU, self.createInfoPassage, id=StoryFrame.STORYSETTINGS_TITLE
        )

        self.storySettingsMenu.Append(
            StoryFrame.STORYSETTINGS_SUBTITLE, "StorySubtitle"
        )
        self.Bind(
            wx.EVT_MENU, self.createInfoPassage, id=StoryFrame.STORYSETTINGS_SUBTITLE
        )

        self.storySettingsMenu.Append(StoryFrame.STORYSETTINGS_AUTHOR, "StoryAuthor")
        self.Bind(
            wx.EVT_MENU, self.createInfoPassage, id=StoryFrame.STORYSETTINGS_AUTHOR
        )
        ...
        self.storySettingsMenu.Append(
            StoryFrame.STORYSETTINGS_SETTINGS, "StorySettings"
        )
        self.Bind(
            wx.EVT_MENU, self.createInfoPassage, id=StoryFrame.STORYSETTINGS_SETTINGS
        )

        self.storySettingsMenu.Append(
            StoryFrame.STORYSETTINGS_INCLUDES, "StoryIncludes"
        )
        self.Bind(
            wx.EVT_MENU, self.createInfoPassage, id=StoryFrame.STORYSETTINGS_INCLUDES
        )

        self.storySettingsMenu.AppendSeparator()

        self.storySettingsMenu.Append(
            StoryFrame.STORYSETTINGS_HELP, "About Special Passages"
        )
        self.Bind(
            wx.EVT_MENU,
        ...
        self.storyMenu.Append(
            StoryFrame.REFRESH_INCLUDES_LINKS, "Update StoryIncludes Links"
        )
        self.Bind(
            wx.EVT_MENU,
            lambda e: self.storyPanel.refreshIncludedPassageList(),
        ...
        for key in sorted(app.headers.keys()):
            ...
            self.Bind(
                wx.EVT_MENU,
                lambda e, target=key: self.setTarget(target),
    ...
    def checkCloseDo(self, event, byMenu):
        ...
        if self.lastTestBuild and os.path.exists(self.lastTestBuild.name):
            try:
                ...
                print(
                    "Failed to remove lastest test build: {}".format(ex),
                    file=sys.stderr,
                )
        self.lastTestBuild = None
    ...
    def exportSource(self, event=None):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            try:
                ...
                order = [
                    widget.passage.title for widget in self.storyPanel.sortedWidgets()
                ]
    ...
    def importSource(self, path, html=False):
        ...
        try:
            ...
            if len(tw.tiddlers):
                ...
                for title in (
                    tw.tiddlers.viewkeys() & self.storyPanel.widgetDict.viewkeys()
                ):

                    dialog = wx.MessageDialog(
                        self,
                        'There is already a passage titled "'
                        + title
                ...
                dialog = wx.MessageDialog(
                    self,
                    "No passages were found in this file. Make sure "
                    + "this is a "
                    + what
                    + " file.",
                    "No Passages Found",
        ...
        try:
            ...
            if maintype != "image":
                self.app.displayError(
                    "importing from the web: The server served "
                    + maintype
                    + " instead of an image",
                    stacktrace=False,
                )
    ...
    def importImageURLDialog(self, event=None):
        dialog = wx.TextEntryDialog(
            self,
            "Enter the image URL (GIFs, JPEGs, PNGs, SVGs and WebPs only)",
    ...
    def importImageDialog(self, event=None, useImageDialog=False, replace=None):
        ...
        if useImageDialog:
            ...
            dialog.ChangeFileTypes(
                [("Web Image File", "*.(gif|jpg|jpeg|png|webp|svg)")]
            )
            dialog.ResetFiles()
        else:
            dialog = wx.FileDialog(
    ...
    def finishImportImage(self, text, title, showdialog=True):
        ...
        if showdialog:
            dialog = wx.MessageDialog(
                self,
                "Image file imported successfully.\n"
...
\tfont-family: "'''
            ...
            if showdialog:
                dialog = wx.MessageDialog(
                    self,
                    "Font file imported successfully.\n"
...
"""
    ...
    def createInfoPassage(self, event):
        ...
        if editingWidget is None:
            editingWidget = self.storyPanel.newWidget(
                title=title, text=self.defaultTextForPassage(title)
            )
    ...
    def verify(self, event=None):
        ...
        if noprobs:
            wx.MessageDialog(
                self,
                "No obvious problems found in "
                + str(len(self.storyPanel.widgetDict))
                + " passage"
        ...
        try:
                    ...
                    def callback(passage, tw=tw):
                        ...
                        # Check for uniqueness
                            ...
                            raise Exception(
                                'A passage titled "'
                                + passage.title
                                + '" is already present in this story'
                            )
                        elif tw.hasTiddler(passage.title):
                            raise Exception(
                                'A passage titled "'
                                + passage.title
                                + '" has been included by a previous StoryIncludes file'
                            )
            ...
            if temp:
                ...
                if self.lastTestBuild and os.path.exists(self.lastTestBuild.name):
                    os.remove(self.lastTestBuild.name)
                path = (
                    (os.path.exists(self.buildDestination) and self.buildDestination)
                    or (os.path.exists(self.saveDestination) and self.saveDestination)
                    or None
                )
                html = tw.toHtml(
                    self.app,
                    header,
                    startAt=startAt,
                    defaultName=self.title,
                    metadata=metadata,
                )
                if html:
                    self.lastTestBuild = tempfile.NamedTemporaryFile(
                        mode="wb",
                        suffix=".html",
                        delete=False,
                    ...
                    if displayAfter:
                        self.viewBuild(name=self.lastTestBuild.name)
            else:
                dest = open(self.buildDestination, "wb")
                dest.write(
                    tw.toHtml(
                        self.app, header, defaultName=self.title, metadata=metadata
                    ).encode("utf-8-sig")
                )
                dest.close()
                if displayAfter:
    ...
    def getLocalDir(self):
        dir = (
            self.saveDestination != "" and os.path.dirname(self.saveDestination)
        ) or None
            ...
            try:
                ...
                if not silent:
                    self.app.displayError(
                        ...
                        + line
                        + '" which is referred to by the StoryIncludes passage',
                        stacktrace=False,
                    )
    ...
    def stats(self, event=None):
        ...
        statFrame = StatisticsDialog(
            parent=self, storyPanel=self.storyPanel, app=self.app
        )
        statFrame.ShowModal()
    ...
    def setTarget(self, target):
        if target not in self.app.headers:
            self.app.displayError(
                "opening the last edited story: the story format '"
                + target
                + "' isn't available.\n"
    ...
    def updateUI(self, event=None):
        ...
        self.menus.FindItemById(StoryFrame.EDIT_FIND_NEXT).Enable(
        ...
        if selections == 1:
            ...
            testItem.SetItemLabel(
                'Test Play From "' + widget.passage.title + '"'
                if widget.passage.isStoryPassage()
                else "Test Play From Here"
            )
        ...
        self.menus.FindItemById(StoryFrame.STORY_EDIT_FULLSCREEN).Enable(
            selections == 1
        )
        self.menus.FindItemById(StoryFrame.BUILD_REBUILD).Enable(
            self.buildDestination != ""
        )
        self.menus.FindItemById(StoryFrame.BUILD_VIEW_LAST).Enable(
            self.buildDestination != ""
        )

        hasStoryIncludes = (
            self.buildDestination != ""
            and "StoryIncludes" in self.storyPanel.widgetDict
        )
        self.autobuildmenuitem.Enable(hasStoryIncludes)
        self.menus.FindItemById(StoryFrame.REFRESH_INCLUDES_LINKS).Enable(
            hasStoryIncludes
        )
    ...
    def serialize(self):
        ...
        return {
            "target": self.target,
            "buildDestination": self.buildDestination,
    ...
    [
        STORY_NEW_PASSAGE,
        STORY_NEW_SCRIPT,
        STORY_NEW_STYLESHEET,
        STORY_NEW_ANNOTATION,
        STORY_EDIT_FULLSCREEN,
        STORY_STATS,
        STORY_METADATA,
        STORY_IMPORT_IMAGE,
        STORY_IMPORT_IMAGE_URL,
        STORY_IMPORT_FONT,
        STORY_FORMAT_HELP,
        STORYSETTINGS_START,
        STORYSETTINGS_TITLE,
        STORYSETTINGS_SUBTITLE,
        STORYSETTINGS_AUTHOR,
        STORYSETTINGS_MENU,
        STORYSETTINGS_SETTINGS,
        STORYSETTINGS_INCLUDES,
        STORYSETTINGS_INIT,
        STORYSETTINGS_HELP,
        REFRESH_INCLUDES_LINKS,
    ] = range(401, 422)
