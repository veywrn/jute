...
class FindPanel(wx.Panel):
    ...
    def __init__(self, parent, onFind=None, onClose=None):
        ...
        buttonSizer.Add(
            self.closeButton, flag=wx.TOP | wx.RIGHT, border=metrics.size("buttonSpace")
        )
        buttonSizer.Add(
            self.findButton, flag=wx.TOP, border=metrics.size("buttonSpace")
        )
        sizer.Add(
            buttonSizer,
...
class ReplacePanel(wx.Panel):
    ...
    def __init__(
        ...
        fieldSizer.Add(
            wx.StaticText(self, label="Replace With"),
            flag=wx.BOTTOM | wx.RIGHT | wx.ALIGN_CENTER_VERTICAL,
            border=metrics.size("relatedControls"),
            proportion=0,
        )
        ...
        optionSizer.Add(
            self.regexpCheckbox,
            flag=wx.BOTTOM | wx.TOP | wx.LEFT,
            border=metrics.size("relatedControls"),
        )
        sizer.Add(
            optionSizer, flag=wx.LEFT | wx.RIGHT, border=metrics.size("windowBorder")
        )
        ...
        buttonSizer.Add(
            self.replaceAllButton, flag=wx.TOP, border=metrics.size("buttonSpace")
        )
    ...
    def onReplaceAll(self, event):
        ...
        if self.replaceAllCallback:
            regexps = self.assembleRegexps()
            self.replaceAllCallback(
                regexps["find"], regexps["flags"], regexps["replace"]
            )
