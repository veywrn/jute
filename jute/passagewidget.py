...
class PassageWidget(object):
    ...
    def findSpace(self):
        ...
        while self.intersectsAny() and turns < 99 * griddivision:
            ...
            self.pos[int(math.floor((turns * 2) % 2))] += (
                self.parent.GRID_SPACING
                * griddivision
                * int(math.copysign(1, turns % 2 - 1))
            )
            movecount -= 1
            if movecount <= 0:
                turns += 0.5
    ...
    def findSpaceQuickly(self):
        ...
        while self.intersectsAny():
            ...
            maxWidth = self.parent.toLogical(
                (self.parent.GetSize().width - self.parent.INSET[0], -1), scaleOnly=True
            )[0]
    ...
    def containsRegexp(self, regexp, flags):
        ...
        """
        return (
            re.search(regexp, self.passage.title, flags) is not None
    ...
    def replaceRegexp(self, findRegexp, replaceRegexp, flags):
        ...
        self.passage.text, textReps = re.subn(
            compiledRegexp, replaceRegexp, self.passage.text
        )
        if titleReps > 0:
    ...
    def getBrokenLinks(self):
        ...
        return list(
            filter(lambda a: not self.parent.passageExists(a), self.passage.links)
        )
    ...
    def getVariableLinks(self):
        ...
        return list(
            filter(
                lambda a: tweelexer.TweeLexer.linkStyle(a) == tweelexer.TweeLexer.PARAM,
                self.passage.links,
            )
        )
    ...
    def openEditor(self, event=None, fullscreen=False):
        ...
        if not hasattr(self, "passageFrame"):
            if image:
                ...
                if fullscreen:
                    self.passageFrame.openFullscreen()
        else:
            try:
                self.passageFrame.Iconize(False)
                self.passageFrame.Raise()
                if fullscreen and not image:
                    self.passageFrame.openFullscreen()
            except RuntimeError:
    ...
    def intersectsAny(self, dragging=False):
        ...
        for widget in (
            self.parent.notDraggingWidgets
            if dragging
            else self.parent.widgetDict.values()
        ):
    ...
    def getConnectorLine(self, otherWidget, clipped=True):
        ...
        if clipped:
            [start, end] = geometry.clipLineByRects(
                [start, end], otherWidget.getLogicalRect()
            )
                ...
                for t in self.passage.tags:
                    if t not in tiddlywiki.TiddlyWiki.INFO_TAGS:
                        for otherWidget in self.parent.taggedWidgets(t):
                            if (
                                not otherWidget.dimmed
                                and not otherWidget.passage.isStylesheet()
                            ):
                                ret.append(otherWidget)
        return ret

    def addConnectorLinesToDict(
        self,
        displayArrows,
        imageArrows,
        flatDesign,
        ...
        widths = (
            max(self.parent.toPixels((widths[0], 0), scaleOnly=True)[0], 2),
        ...
        if widgets:
            for widget in widgets:
                ...
                if self.passage.isAnnotation():
                    ...
                    link in self.passage.displays + self.passage.macros
                ) and link not in self.passage.links:
                ...
                line, arrow = self.getConnectorTo(
                    widget, not arrowDictonary is None, updateRect
                )
    ...
    def getConnectorTo(self, otherWidget, arrowheads=False, updateRect=None):
        ...
        if not arrowheads:
            ...
            length = max(
                self.parent.toPixels(
                    (PassageWidget.ARROWHEAD_LENGTH, 0), scaleOnly=True
                )[0],
                1,
            )
    ...
    def paint(self, dc):
        ...
        if (not self.paintBufferBounds) or (
            rect.width != self.paintBufferBounds.width
    ...
    def getTitleColor(self):
        ...
        if self.passage.isAnnotation():
            ...
            not self.linksAndDisplays()
            and not self.getIncludedLinks()
    ...
    def cachePaint(self, size):
        ...
        def dim(c, dim, flat=flat):
            ...
            if dim:
                a = (
                    PassageWidget.FLAT_DIMMED_ALPHA
                    if flat
                    else PassageWidget.DIMMED_ALPHA
                )
            ...
            c = [max(0, min(255, int(i))) for i in c]
            return wx.Colour(*c)
        ...
        gc = (
            ...
            else wx.GraphicsContext.Create(self.paintBuffer)
        )

        # text font sizes
        # wxWindows works with points, so we need to doublecheck on actual pixels

        titleFontSize = self.parent.toPixels(
            (metrics.size("widgetTitle"), -1), scaleOnly=True
        )[0]
        titleFontSize = sorted(
            (metrics.size("fontMin"), titleFontSize, metrics.size("fontMax"))
        )[1]
        excerptFontSize = sorted(
            (metrics.size("fontMin"), titleFontSize * 0.9, metrics.size("fontMax"))
        )[1]

        if self.app.config.ReadBool("flatDesign"):
            titleFont = wx.Font(
                titleFontSize, wx.SWISS, wx.NORMAL, wx.LIGHT, False, "Arial"
            )
            excerptFont = wx.Font(
                excerptFontSize, wx.SWISS, wx.NORMAL, wx.LIGHT, False, "Arial"
            )
        else:
            titleFont = wx.Font(
                titleFontSize, wx.SWISS, wx.NORMAL, wx.BOLD, False, "Arial"
            )
            excerptFont = wx.Font(
                excerptFontSize, wx.SWISS, wx.NORMAL, wx.NORMAL, False, "Arial"
            )
        ...
        tagBarColor = dim(
            tuple(
                ...
                for i in colorsys.hsv_to_rgb(
                    0.14 + math.sin(hash("".join(self.passage.tags))) * 0.08,
        ...
        tags = set(self.passage.tags) - (
            tiddlywiki.TiddlyWiki.INFO_TAGS | self.getHeader().invisiblePassageTags()
        )
        ...
        if self.passage.isAnnotation():
            ...
            frameInterior = (
                dim(colors["bodyStart"], self.dimmed),
            ...
            if isinstance(gc, wx.GraphicsContext):
                gc.SetBrush(
                    gc.CreateLinearGradientBrush(
                        0, 0, 0, size.height, frameInterior[0], frameInterior[1]
                    )
                )
            else:
                gc.GradientFillLinear(
                    wx.Rect(0, 0, size.width - 1, size.height - 1),
                    frameInterior[0],
                    frameInterior[1],
                    wx.SOUTH,
                )
        ...
        # title bar
        titleBarHeight = (
            PassageWidget.GREEK_HEIGHT * 3 if greek else titleFontHeight + (2 * inset)
        )
        if self.passage.isAnnotation():
        ...
        if not greek:
            ...
            if isinstance(gc, wx.GraphicsContext):
                ...
                gc.SetClippingRegion(
                    wx.Rect(inset, inset, size.width - (inset * 2), titleBarHeight - 2)
                )
            ...
            if not self.passage.isImage():
                ...
                for line in excerptLines:
                    ...
                    excerptTop += (
                        excerptFontHeight
                        * PassageWidget.LINE_SPACING
                        * min(
                            1.75,
                            max(
                                1,
                                1.75 * size.width / 260
                                if (self.passage.isAnnotation() and line)
                                else 1,
                            ),
                        )
                    )
            ...
            if (self.passage.isStoryText() or self.passage.isStylesheet()) and tags:
                ...
                gc.DrawRectangle(
                    0, size.height - tagBarHeight - 1, size.width, tagBarHeight + 1
                )
        ...
        if self.passage.isImage():
            if self.bitmap:
                if isinstance(gc, wx.GraphicsContext):
                    ...
                    gc.DestroyClippingRegion()
                    gc.SetClippingRegion(
                        wx.Rect(1, titleBarHeight + 1, size.width - 3, size.height - 3)
                    )
                ...
                scale = min(
                    width / float(self.bitmap.GetWidth()),
                ...
                if scale != 1:
                    img = img.Scale(
                        scale * self.bitmap.GetWidth(), scale * self.bitmap.GetHeight()
                    )
                ...
                if isinstance(gc, wx.GraphicsContext):
                    gc.DrawBitmap(
                        img.ConvertToBitmap(self.bitmap.GetDepth()),
                        1 + offsetWidth,
        ...
        def showEmblem(emblem, gc=gc, size=size, inset=inset):
            ...
            emblemPos = [
                size.width - (emblemSize[0] + inset),
                size.height - (emblemSize[1] + inset),
            ]

            if isinstance(gc, wx.GraphicsContext):
                gc.DrawBitmap(
                    emblem, emblemPos[0], emblemPos[1], emblemSize[0], emblemSize[1]
                )
            else:
    ...
    def serialize(self):
        ...
        return {
            ...
            "pos": self.pos,
            "passage": copy.copy(self.passage),
        }
...
class PassageWidgetContext(wx.Menu):
    def __init__(self, parent):
        ...
        if parent.passage.isStoryPassage():
            ...
            self.Bind(
                wx.EVT_MENU,
                lambda e: self.parent.parent.parent.testBuild(
                    startAt=parent.passage.title
                ),
                id=test.GetId(),
            )
        ...
        self.Bind(
            wx.EVT_MENU,
            lambda e: self.parent.parent.removeWidget(self.parent.passage.title),
            id=delete.GetId(),
        )
