...
class TiddlyWiki(object):
    ...
    def toHtml(
        self, app, header=None, order=None, startAt="", defaultName="", metadata={}
    ):
        ...
        if not header:
            app.displayError(
                "building: no story format was specified.\n"
        ...
        tz_offset = (
            lambda t: "%s%02d%02d"
            % (("+" if t <= 0 else "-",) + divmod(abs(t) / 60, 60))
        )(time.timezone)
        # Obtain the encoding expected to be used by strftime in this locale
        strftime_encoding = (
            locale.getlocale(locale.LC_TIME)[1] or locale.getpreferredencoding()
        )
        # Write the timestamp
        output = output.replace(
            '"TIME"',
            "Built on {}".format(time.strftime("%d %b %Y at %H:%M:%S, " + tz_offset)),
        )

        # Insert the test play "start at passage" value
        if startAt:
            output = output.replace(
                '"START_AT"', '"' + startAt.replace("\\", r"\\").replace('"', '"') + '"'
            )
        else:
        ...
        jquery = (
            "jquery" in self.storysettings and self.storysettings["jquery"] != "off"
        )
        modernizr = (
            "modernizr" in self.storysettings
            and self.storysettings["modernizr"] != "off"
        )
        blankCSS = (
            "blankcss" in self.storysettings and self.storysettings["blankcss"] != "off"
        )

        for i in filter(
            lambda a: (a.isScript() or a.isStylesheet()), self.tiddlers.values()
        ):
            if (
                not jquery
                and i.isScript()
                and re.search(r"requires? jquery", i.text, re.I)
            ):
                jquery = True
            ...
            if (
                not blankCSS
                and i.isStylesheet()
                and re.search(r"blank stylesheet", i.text, re.I)
            ):
                blankCSS = True
        ...
        if jquery:
            output = insertEngine(
                app, output, app.builtinTargetsPath + "jquery.js", '"JQUERY"'
            )
            if not output:
                return
        else:
        ...
        if modernizr:
            output = insertEngine(
                app, output, app.builtinTargetsPath + "modernizr.js", '"MODERNIZR"'
            )
            if not output:
                return
        else:
        ...
        if blankCSS:
            ...
            output = re.sub(
                r'<style\s+id=["\']?defaultCSS["\']?\s*>(?:[^<]|<(?!\/style>))*<\/style>',
                "",
                output,
                flags=re.I | re.M,
                count=1,
        ...
        rot13 = (
            "obfuscate" in self.storysettings
            and self.storysettings["obfuscate"] != "off"
        )
        ...
        for i in order:
            ...
            if self.NOINCLUDE_TAGS.isdisjoint(tiddler.tags):
                storyfragments.append(
                    tiddler.toHtml(rot13 and tiddler.isObfuscateable())
                )
        storycode = u"".join(storyfragments)

        if output.count('"STORY_SIZE"') > 0:
            output = output.replace(
                '"STORY_SIZE"', '"' + str(len(storyfragments)) + '"'
            )
    ...
    def toRtf(self, order=None):
        ...
        output += (
            r"{\fonttbl\f0\fswiss\fcharset0 Arial;\f1\fmodern\fcharset0 Courier;}"
            + "\n"
        )
        ...
        output += (
            r"\pard\tx720\tx1440\tx2160\tx2880\tx3600\tx4320\tx5040\tx5760\tx6480\tx7200\tx792"
            + "\n"
        )
        ...
        for i in order:
            ...
            text = re.sub(r"\n", "\\\n", text)  # newlines
            text = re.sub(
                tweeregex.LINK_REGEX, r"\\b\\cf2 \\ul \1\\ulnone \\cf0 \\b0 ", text
            )  # links
            ...
            text = re.sub(
                tweeregex.HTML_REGEX, r"\\f1\\cf1 \g<0>\\cf0\\f0 ", text
            )  # macros
            ...
            output += (
                r"\fs24\b1 "
                + rtf_encode(self.tiddlers[i].title)
                + r"\b0\fs20 "
                + "\\\n"
            )
            output += text + "\\\n\\\n"
    ...
    def addHtml(self, source):
        ...
        order = []
        divs = re.search(
            r'<div\s+id=(["\']?)store(?:A|-a)rea\1(?:\s+data-size=(["\']?)\d+\2)?(?:\s+hidden(?:=["\']{2})?)?\s*>(.*)</div>',
            source,
            re.DOTALL,
        )
        if divs:
    ...
    INFO_TAGS = (
        frozenset(["script", "stylesheet", "annotation"])
        | SPECIAL_TAGS
        | NOINCLUDE_TAGS
    )
...
class Tiddler:  # pylint: disable=old-style-class
    ...
    def isStoryText(self):
        ...
        return (
            self.title not in TiddlyWiki.UNFORMATTED_INFO_PASSAGES
    ...
    def isStoryPassage(self):
        ...
        return (
            self.title not in TiddlyWiki.INFO_PASSAGES
            and TiddlyWiki.INFO_TAGS.isdisjoint(self.tags)
        )
    ...
    def update(self):
        ...
        """
        if (
            not self.isStoryText()
            and not self.isAnnotation()
        ...
        self.displays = list(
            set(
                re.findall(
                    r'\<\<display\s+[\'"]?(.+?)[\'"]?\s?\>\>', self.text, re.IGNORECASE
                )
            )
        )
        ...
        for m in re.finditer(tweeregex.HTML_REGEX, self.text):
            attrs = m.group(2)
            if attrs:
                dataPassage = re.search(
                    r"""data-passage\s*=\s*(?:([^<>'"=`\s]+)|'((?:[^'\\]*\\.)*[^'\\]*)'|"((?:[^"\\]*\\.)*[^"\\]*)")""",
                    attrs,
                )
                if dataPassage:
                    link = (
                        dataPassage.group(1)
                        or dataPassage.group(2)
                        or dataPassage.group(3)
                    )
                    if m.group(1) == "img":
                        images.add(link)
                    else:
