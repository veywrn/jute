...
class StatisticsDialog(wx.Dialog):
    ...
    def __init__(self, parent, storyPanel, app, id=wx.ID_ANY):
        ...
        countPanelSizer = wx.FlexGridSizer(
            6, 2, metrics.size("relatedControls"), metrics.size("relatedControls")
        )
    ...
    def count(self, panel):
        ...
        def count(widget, counts):
            if widget.passage.isStoryText():
                ...
                iterator = re.finditer(
                    tweeregex.MACRO_REGEX + "|" + tweeregex.LINK_REGEX,
                    widget.passage.text,
                    re.U | re.I,
                )
                for p in iterator:
                    iterator2 = re.finditer(
                        tweeregex.MACRO_PARAMS_REGEX, p.group(0), re.U | re.I
                    )
                    for p2 in iterator2:
                        if p2.group(4):
        ...
        if len(variables):
            ...
            variablesCtrl = wx.TextCtrl(
                panel,
                -1,
                size=(StatisticsDialog.MIN_WIDTH * 0.9, 60),
        ...
        if len(tags):
            ...
            self.panelSizer.Add(
                wx.StaticText(panel, label=str(len(tags)) + " Tags"),
