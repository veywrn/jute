    ...
    def filesToEmbed(self):
        ...
        return OrderedDict(
            [
                ...
                (
                    '"SUGARCANE"',
                    self.builtinPath + os.sep + "sugarcane" + os.sep + "code.js",
                ),
    ...
    def passageChecks(self):
        ...
        def checkInequalityExpression(tag, start, end, style, passage=None):
            if style == tweelexer.TweeLexer.MACRO:
                r = re.search(
                    r"\s+((and|or|\|\||&&)\s+([gl]te?|is|n?eq|(?:[=!<]|>(?!>))=?))\s+"
                    + tweeregex.UNQUOTED_REGEX,
                    tag,
                )
                if r:
                    yield (
                        ...
                        + 'There should probably be an expression, or a variable, between "'
                        + r.group(2)
                        + '" and "'
                        + r.group(3)
                        + '".',
        ...
        def checkIfMacro(tag, start, end, style, passage=None):
            if style == tweelexer.TweeLexer.MACRO:
                ifMacro = re.search(
                    tweeregex.MACRO_REGEX.replace(r"([^>\s]+)", r"(if\b|else ?if\b)"),
                    tag,
                )
                if ifMacro:
                    # Check that the single = assignment isn't present in an if/elseif condition.
                    r = re.search(
                        r"([^=<>!~])(=(?!=))(.?)" + tweeregex.UNQUOTED_REGEX, tag
                    )
                    if r:
                        warning = (
                            tag
                        ...
                        yield (
                            warning,
                            (start + r.start(2), insertion, start + r.end(2)),
                        )
        ...
        def checkHTTPSpelling(tag, start, end, style, passage=None):
            if style == tweelexer.TweeLexer.EXTERNAL:
                ...
                if regex:
                    yield (
                        r"You appear to have misspelled 'http"
                        + regex.group(1)
                        + "://'.",
                        (
                            start + regex.start(0),
        ...
        def checkScriptTagsInScriptPassage(passage):
            ...
            ret = []
            scriptTags = re.finditer(
                r"(?:</?script\b[^>]*>)" + tweeregex.UNQUOTED_REGEX, passage.text
            )
        ...
        return (
            [
                checkUnmatchedMacro,
                checkInequalityExpression,
                checkIfMacro,
                checkHTTPSpelling,
            ],
            [],
            [checkScriptTagsInScriptPassage],
        )
